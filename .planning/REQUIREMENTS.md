...
- [ ] **TIF-13**: When TIF is active, each historical window includes a rolling-window variant (last N days, configurable via `tifRollingDays` setting); when MA/AA values are recorded on a day, those are used in preference to derived timestamp differences
- [ ] **TIF-15**: When TIF is active, each window computes a median in addition to its min/max bounds; the central predicted time for each event type is the average of the medians across all active windows (the min/max band bounds continue to use intersection/union of window ranges unchanged)
