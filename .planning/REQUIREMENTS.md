...
- [x] **PRED-13**: `js/lib/forecast-blend.js` exports `blendForecast(dayRecords, snap)` — Algorithm C entry point returning the same top-level prediction shape as Classic and TIF so `today-screen.js` can swap between all three transparently
...
- [x] **PRED-15**: Algorithm C bedtime prediction — three-band blend: Historic bedtime band + Day-length band + Activity-after-nap band (AA band); central = average of the three trimmed medians
...
|-------------|---------|-------|--------|
| PRED-13 | B-050 | Phase 25 | Complete |
| PRED-14 | B-050 | Phase 25 | Complete |
| PRED-15 | B-050 | Phase 25 | Complete |
