...
//
// Testability seam (added Plan 01-05, Task 1):
//   - Optional second argument `ls` defaults to `globalThis.localStorage`.
//     Pure-Node tests inject a fake `{ getItem, setItem }` object so the
//     T-03 corrupted-blob + T-04 QuotaExceededError translation paths
//     are observable without polluting globals. The runtime call site
//     (`createStorageLocal('nightwatch:db')` in `js/app.js`) remains
//     unchanged — backward-compatible signature change.

/**
 * @param {string} key storage key passed by the composition root (D-02)
 * @param {{ getItem: (k: string) => (string|null), setItem: (k: string, v: string) => void }} [ls]
 *   optional localStorage-like injection; defaults to `globalThis.localStorage`
 *   in browsers (and undefined in pure Node — tests MUST inject a fake)
 * @returns {{ load: () => object|null, save: (db: object) => void }}
 */
export function createStorageLocal(key, ls = globalThis.localStorage) {
  return {
    load() {
      try {
